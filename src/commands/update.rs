...
use crate::{
    ...
    state,
    structs::{common::{Command, CommandData, QueryResult, Value}, custom::{dms::Dms, friends::Friends, guild_members::{GuildMembers, GuildMembersCQL}, messages::Messages, roles::{Roles, RolesCQL}, settings::{CqlSettings, Settings}, udt::{BigintPair, BigintPairUDT, GuildOrderType, GuildOrderTypeUDT, MemberTimeouts, MemberTimeoutsUDT, MentionsType, MentionsTypeUDT, TokenType, TokensTypeUDT}}, update::UpdateResponse},
...
pub async fn update(
    ...
    match command {
        CommandData::Update(update_data) => {
            ...
            if raw_command.type_.is_some() {
                let json: String = serde_json::to_string(&update_data.values).unwrap();
